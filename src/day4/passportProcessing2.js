function parsePassports(batchData) {
  let passports = batchData.split('\n\n')
  passports = passports.map(passport => passport.replaceAll("\n", " "))
  return passports.map(passport => {
    const passportDetails = {}
    passport.split(" ").forEach(keyValuePair => {
      const [key, value] = keyValuePair.split(":")
      passportDetails[key] = value
    })
...
const validators = {
  ...
  "hcl": (hcl) => {
    const match = hcl.match(/#[a-f0-9]{6}/g)
    return match && match.length > 0 && hcl.length === 7
  },
  "ecl": (ecl) => {
    return ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].includes(ecl)
  },
  "pid": (pid) => {
    const match = pid.match(/[0-9]{9}/g)
    return match && match.length > 0 && pid.length === 9
  }
}

function isValid(passport) {
  const requiredFields = Object.keys(validators)
  const doesContainAllRequiredFields = requiredFields.every(field => Object.keys(passport).includes(field))
  console.log(passport)
  return doesContainAllRequiredFields && requiredFields.every(validator => {
    return validators[validator](passport[validator])
  })
}

function countValidPassports(passports) {
  return passports.reduce((count, passport) => {
    return count + isValid(passport)
  }, 0)
}

const main = () => {
  /*const batchData = `ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
iyr:2011 ecl:brn hgt:59in`*/
  ...
  const batchData = `iyr:1928 cid:150 pid:476113241 eyr:2039 hcl:a5ac0f
ecl:#25f8d2
byr:2027 hgt:190
...
byr:1950`
  ...
  const passports = parsePassports(batchData)
