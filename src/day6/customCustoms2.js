const isContainCharInAllElement = (answers, char) => answers.every(answer => answer.includes(char));

const countCorrectAnswers = answers => {
  const answersOfFirstPerson = answers[0].split('')
  return answersOfFirstPerson.reduce((count, char) => {
      return count + isContainCharInAllElement(answers, char)
    }, 0
  )
};

const calculateCorrectAnswers = rawAnswers => {
  const answers = rawAnswers.split("\n\n");
  const answersByEachPerson = answers.map(answer => answer.split('\n'))
  return answersByEachPerson.map(countCorrectAnswers)
}

const main = () => {
  /*const rawAnswers = `abc
...
zbwlnqrc
zwrpc`
  const answers = calculateCorrectAnswers(rawAnswers)
