const unique = text => {
  return text.split('').reduce((txt, char) => {
    if (!txt.includes(char)) txt += char
    return txt
  }, '')
}
...
const parseAnswers = rawAnswers => {
  ...
  const answersWithOutNewLine = answers.map(answer => answer.replaceAll('\n', ''))
  return answersWithOutNewLine.map(answer => unique(answer))
}
...
`
  ...
  console.log(answers.join('').length)
}
