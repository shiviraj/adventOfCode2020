function calculateOccurrence(password, char) {
  return password.split(char).length - 1

}

function isCorrectPassword(word) {
  const [range, character, password] = word.split(' ')
  const [char] = character.split(":")
  const [min, max] = range.split("-").map(num => +num)
  const occurrence = calculateOccurrence(password, char)
  return occurrence >= min && occurrence <= max
}

function correctPasswords(passwords) {
  return passwords.reduce((numOfCorrectPasswords, password) => {
    return numOfCorrectPasswords + isCorrectPassword(password)
  }, 0)
}
...
4-5 c: ccchc`.split('\n')
  console.log(correctPasswords(passwords))
}
