function isCorrectPassword(word) {
  const [range, character, password] = word.trim().split(' ')
  const [char] = character.split(":")
  const [min, max] = range.split("-").map(num => +num)
  if (password[min - 1] === char && password[max - 1] === char) return 0
  else if (password[min - 1] === char || password[max - 1] === char) return 1
  else return 0
}
